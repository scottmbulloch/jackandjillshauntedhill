...
//========================================================================================================
// Email update on form submission
//========================================================================================================
  
  if($sbName || $sbEMail || $sbText) {
    $to="jackandjillshauntedhill@gmail.com";
    mail($to, "COMMENT BOX SUMISSION", "$sbName: $sbText", "From: $sbEMail");
  }
...
//========================================================================================================
  ...
  function read_data() {
    ...
    if(file_exists('shoutbox.txt')) {
      $size = filesize('shoutbox.txt');

      if($size > 0) {
        if($fp = fopen('shoutbox.txt', 'r')) {
          $data = fread($fp, $size);
          $data = explode(chr(8) . "\r\n", $data);
...
?>
      <div style="background-color:<? echo $bgcolor; ?>;font-family:monospace;font-size:.75em;color:#222;text-align:right;">
      <? echo $tstamp; ?>
      </div>
      <div class="<? echo $class; ?>" style="background-color:<? echo $bgcolor; ?>">
      <? // if($email) echo '<a href="mailto:' . $email . '">'; ?> 
      <strong style="color:#222;"><? echo $name; ?>:</strong><? if($email) echo '</a>'; ?> <? echo $text; ?>
      </div>
      <hr style="display:block;height:1px;border:0;border-top:1px solid #222;" />
<?
    }
  }
...
?>
    ...
    <body style="background-color:<? echo $messageBGColors[0]; ?>">
<?
